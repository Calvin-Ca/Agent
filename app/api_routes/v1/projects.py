"""RESTful project routes — CRUD without NL parsing."""
...
async def update_project(project_id: str, body: ProjectUpdate, db: DBSession, user: CurrentUser):
    project = await project_crud.get(db, id=project_id)
    if project is None or project.owner_id != user.id:
        raise NotFoundError("项目不存在")
...
async def delete_project(project_id: str, db: DBSession, user: CurrentUser):
    ...
    return R.ok(message=f"项目「{project.name}」已删除")
