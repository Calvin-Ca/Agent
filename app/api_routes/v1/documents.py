"""RESTful document upload routes."""
...
@router.post("", response_model=R[UploadOut], summary="上传文档")
async def upload_document(
    ...
    if file_type is None:
        raise BizError(message=f"不支持的文件类型: {content_type}，支持: PDF, JPG, PNG, WebP, TXT, MD, CSV")

    content = await file.read()
    if len(content) > _MAX_FILE_SIZE:
        raise BizError(message="文件大小超过 50MB 限制。")
    ...
    return R.ok(data=UploadOut.model_validate(doc), message="上传成功，正在后台处理")
